...
module.exports = function (_Generator) {
    ...
    function JsGenerator() {
        ...
        _this.option('react-hot-reload', {
            type: Boolean,
            defaults: false
        });

        _this.option('babel-compile', {
            type: Boolean,
            defaults: false
        });

        _this.option('babel-exclude-runtime', {
            type: Boolean,
            defaults: false
        });
                ...
                prompts: function prompts() {
                    ...
                    prompts.push({
                        ...
                        when: function when(answers) {
                            var type = _this2.options.type || answers.type;
                            return type === 'react';
                        }
                    });
        ...
        get: function get() {
            return {
                directory: function directory() {
                    ...
                    var srcPath = this.templatePath(this.options.type);
                ...
                babelrc: function babelrc() {
                    var projectPath = _lodash2.default.get(this.options, 'project-path');
                    var reactHotReloading = _lodash2.default.get(this.options, 'react-hot-reload');
                    var babelCompile = _lodash2.default.get(this.options, 'babel-compile');
                    var babelExcludeRuntime = _lodash2.default.get(this.options, 'babel-exclude-runtime');
        ...
        get: function get() {
            return {
                npm: function npm() {
                    ...
                    this.yarnInstall(['babel-plugin-add-module-exports@latest', 'babel-preset-airbnb@latest', 'domready@latest', 'fastclick@latest', 'hoist-non-react-statics@latest', 'hypernova@latest', 'immutable@latest', 'invariant@latest', 'keymirror@latest', 'lodash@latest', 'react@latest', 'prop-types@latest', 'react-dom@latest', 'react-redux@latest', 'history@3.0', 'react-router@3.0', 'react-router-scroll@latest', 'react-router-redux@4.0', 'redux@latest', 'redux-thunk@latest', 'redux-logger@latest', 'redux-promise@latest', 'redux-devtools@latest', 'redux-devtools-log-monitor@latest', 'redux-devtools-dock-monitor@latest']);

                    if (this.options['webpack-hot-reload']) {
                        this.yarnInstall(['react-hot-loader@^3.0.0-beta.7'], {
                            dev: true
                        });
                    }

                    if (this.react_features.indexOf('relay') !== -1) {
                        this.yarnInstall(['react-relay@latest']);

                        this.yarnInstall(['babel-relay-plugin@latest', 'babel-plugin-transform-relay-hot@latest'], {
                            dev: true
                        });
                    }

                    this.yarnInstall(['babel-eslint@latest', 'eslint@3.19.0', 'eslint-config-airbnb@15.0.2', 'eslint-plugin-import@2.6.1', 'eslint-plugin-jsx-a11y@5.1.1', 'eslint-plugin-react@7.1.0', 'html-webpack-plugin@latest'], {
                        dev: true
                    });
                }
            };
